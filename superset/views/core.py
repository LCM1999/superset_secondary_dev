...
from superset import (
    ...
    event_logger,
    get_feature_flags,
    is_feature_enabled,#原函数层层调用最源头是在utils文件夹中的feature_flag_manager.py中的同名函数
    result_set,
    results_backend,
...
def check_datasource_perms(
    ...
    viz_obj = get_viz(  # type: ignore 构建视图对象并传入参数
...
#BaseSupersetView只是定义了一个基本的json_response用来构造json形势的响应
...
class Superset(BaseSupersetView):
    ...
    def get_json_results(self,viz_obj):
        return self.json_response(
            {"data":viz_obj.get_df_payload()["df"]}
        )
    ...
    def get_samples(self, viz_obj):
        if viz_obj.datasource.database.database_kind:
            ...
            print('打印的是经典款',{"data": viz_obj.get_samples()})
    ...
    def generate_json(
        ...
        if results:
            if viz_obj.datasource.database.database_kind:#图数据库前端显示的数据不能成表就直接返回字典数据然后修改前端显示
                return self.get_json_results(viz_obj)
            else:
                return self.get_raw_results(viz_obj)#图表处观看结果的东西
    ...
    def explore_json(self, datasource_type=None, datasource_id=None):
        ...
        为form_data的GET、POST请求服务

        This endpoint evolved to be the entry point of many different
        requests that GETs or POSTs a form_data.

        此端点演变为 GET 或 POST form_data的许多不同请求的切入点。

        `self.generate_json` receives this input and returns different
        payloads based on the request args in the first block

        TODO: break into one endpoint for each return shape"""
        #获取地址栏？后面的参数,全都是false
        csv = request.args.get("csv") == "true"
        query = request.args.get("query") == "true"
        results = request.args.get("results") == "true"
        samples = request.args.get("samples") == "true"
        force = request.args.get("force") == "true"
        form_data = get_form_data()[0]#没有找到这个get_form_data函数的定义？
        print('打印的表单信息',form_data)
        #print('打印的一堆【判断信息',csv,query,results,samples,force)
        #print('打印的奇怪的函数',get_form_data())
    ...
    def explore(self, datasource_type=None, datasource_id=None):
        ...
        #添加新chart切片时候的url响应函数
        print('user_id:',user_id,'from_data:',form_data,'slc',slc)
        ...
        #如果该片属于当前用户，并且没有被更新，则Flash SIP-15消息已更新，即没有使用[start, end]间隔。
        if (
            config["SIP_15_ENABLED"]#True
            and slc
        ...
        error_redirect = "/chart/list/"#出错后的重新导向
        try:
            datasource_id, datasource_type = get_datasource_info(
                datasource_id, datasource_type, form_data
            )
        ...
        # merge request url params 合并请求url参数
        ...
        bootstrap_data = {
            ...
            "standalone": standalone,#这个是什么， 
            "user_id": user_id,
            "forced_height": request.args.get("height"),
            "common": common_bootstrap_payload(),#???这个是什么
        }
        table_name = (
    ...
    def _sql_json_sync(
        ...
        执行查询
        :param rendered_query: The rendered query (included templates)呈现的查询语句
        :param query: The query SQL (SQLAlchemy) object
        :return: String JSON response
        """
        try:
            timeout = config["SQLLAB_TIMEOUT"]#30
            timeout_msg = f"The query exceeded the {timeout} seconds timeout."
            store_results = (
                is_feature_enabled("SQLLAB_BACKEND_PERSISTENCE")#false
                and not query.select_as_cta#false
            )#false
            with utils.timeout(seconds=timeout, error_message=timeout_msg):#这地方是个坑，在windos中根本没用，也检测不到超时，不超时也会被拖到无限超时
                # pylint: disable=no-value-for-parameter
                data = sql_lab.get_sql_results(#函数在superset文件夹里的sql_lab.py中
                    query.id,
                    rendered_query,
                    return_results=True,
            ...
            logger.exception(f"Query {query.id}: {e}")
            return json_error_response(f"{{查询超时了}}")
    ...
    def sql_json_exec(
        ...
        schema: str = cast(str, query_params.get("schema"))#图数据库默认数据库neo4j
        sql: str = cast(str, query_params.get("sql"))
        #获取form数据
        try:#这个是空的，form表单里没有这一项
            ...
            template_params = {}
        limit: int = query_params.get("queryLimit") or app.config["SQL_MAX_ROW"]#查询行数限制
        async_flag: bool = cast(bool, query_params.get("runAsync"))#默认False
        if limit < 0:
            logger.warning(
                f"Invalid limit of {limit} specified. Defaulting to max limit."
            )
            limit = 0
        select_as_cta: bool = cast(bool, query_params.get("select_as_cta"))#false
        tmp_table_name: str = cast(str, query_params.get("tmp_table_name"))#""
        client_id: str = cast(
            str, query_params.get("client_id") or utils.shortid()[:10]#取到第十个字符
        )#"94Vm3YWN-"
        sql_editor_id: str = cast(str, query_params.get("sql_editor_id"))#"jaEN0XQGTZ"貌似也是一个唯一标识符
        tab_name: str = cast(str, query_params.get("tab"))#自定义的查询名称
        status: str = QueryStatus.PENDING if async_flag else QueryStatus.RUNNING#QueryStatus from models/core

        session = db.session()
        mydb = session.query(models.Database).get(database_id)#返回的是database_name
        #print('这是数据库',mydb)
        if not mydb:
        ...
        try:
            session.add(query)#向mysql数据表中添加字段
            session.flush()
            query_id = query.id
        ...
        rejected_tables = security_manager.rejected_tables(sql, mydb, schema)#拒绝table？
        #print('打印产生的列表',rejected_tables)不安全的话他返回一个空列表[]
        if rejected_tables:
            query.status = QueryStatus.FAILED
            session.commit()
            return json_error_response(
        ...
        try:
            ...
            )#这把dbs和querys两个表的orm对象给了这个g_t_p函数,返回BaseTemplateProcessor类在jinjia_context当中
        ...
        expand_data: bool = cast(
            bool,
            is_feature_enabled("PRESTO_EXPAND_DATA")#False
            and query_params.get("expand_data"),#true
        )#最终的值应该是False

        # Async request.
        if async_flag:#默认false不执行
            return self._sql_json_async(
                session, rendered_query, query, expand_data, log_params
            )
        # Sync request.
        #print('这是打印的render——query',rendered_query)#打印出来同写的语句 MATCH (n:`香港大公网`) RETURN n LIMIT 25
        return self._sql_json_sync(#这个很可能是返回数据的部分
            session, rendered_query, query, expand_data, log_params
        )
