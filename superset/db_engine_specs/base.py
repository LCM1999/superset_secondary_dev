...
class BaseEngineSpec:  # pylint: disable=too-many-public-methods
    ...
    #图数据库传入的engine是GraphSevers
        ...
        print('爱上了大家好房间拿',quote)
        if schema:
            full_table_name = quote(schema) + "." + quote(table_name)
        else:
            full_table_name = quote(table_name)
        #print('全部的表名',full_table_name)和传入的表名没得啥子区别
        #下面这个构建了一个SQL查询语句
        ...
        if latest_partition:
            ...
            if partition_query is not None:
                qry = partition_query
        #print('这是查询语句',qry)
        sql = database.compile_sqla_query(qry)#根据方言组合成一个sql，一般是时限之类的写法有区别的东西
        #print('这个是sql',sql)
        if indent:
            sql = sqlparse.format(sql, reindent=True)#重新缩进
        #print('这是处理完的sql',sql)
        return sql
    ...
    def modify_url_for_impersonation(
        ...
        使用要模拟的用户修改SQL Alchemy URL对象(如果适用的话)
    ...
    def get_extra_params(database: "Database") -> Dict[str, Any]:
        ...
        有些数据库需要向连接参数添加元素，比如将证书传递给' extra '。这可以在这里完成。
