...
class BaseViz:
    ...
    credits = "" #参考对象链接。NVD的那个类在Base的基础上加上了NVD的官方接口连接
    ...
    def __init__(
        ...
        # from_data 是一个字典，目测是从前端图表构建界面左侧组件回传的表单数据
        ...
        # timedelta是pandas的数据格式之一；datatime.timedelta（）=》两个时间的时间差
        self.time_shift = timedelta()
        # Optional[str] = None表示这个数据格式不是字符串就是None
        ...
        self.process_metrics() #处理刻度值的函数

    def process_metrics(self):
        # metrics in TableViz is order sensitive, so metric_dict should be
        # OrderedDict 刻度值字典应该是一个有序字典
        self.metric_dict = OrderedDict()
        fd = self.form_data
        for mkey in METRIC_KEYS:#在70行定义的一个全局变量
            val = fd.get(mkey) #获取表单数据中的mkey（这个数据可能是构成图表插件的必要元素）
            if val:
                if not isinstance(val, list):
                    val = [val]#转化成一个列表型的数据
                for o in val:#获取字符串的刻度值本身作为dict的key，刻度值作为value存储在metric_dict中
                    label = utils.get_metric_name(o)
                    self.metric_dict[label] = o

        # Cast to list needed to return serializable object in py3
        #投到列表中返回py3中可串行对象所需的列表
    ...
    @staticmethod #只能当一个方法函数使用，传给他一个值得到一个结果
    ...
    def run_extra_queries(self):
        ...
        在这些情况下，我们需要确保这些查询在主"get_payload"方法被调用之前运行，
        以便整个缓存元数据可以正确。它在这里的工作方式是，如果以前的任何
        "get_df_payload"呼叫击中缓存，主要有效载荷的元数据将返回这个函数。
        ...
        多查询支持可能需要做更多的工作才能成为框架中的一流使用案例，并且 UI 能够反
        映其中的细微之处（例如，仅显示缓存中只送达了某些查询）。同时，由于多查询是
        罕见的，我们对待它有点黑客。请注意，当从缓存可视化数据本身转向缓存基础查询
        （ies）时，黑客变得有必要。
        """
        pass

    def apply_rolling(self, df):
        #处理表单数据中的rolling_type，_periods,min_period等数据，返回的貌似是创建div大小的指令dict
        fd = self.form_data
    ...
    def get_samples(self):
        #重构组成的基本query语句，并传给233行的下一个函数get_df，返回？
        query_obj = self.query_obj()
        query_obj.update(
            {
                "groupby": [],#sql语句中的排序,附带了查询这个字段的所有信息
                "metrics": [],#聚合材料，这个材料只能选求和计数之类的
                "row_limit": 100000,#数据最大行数设定？还是指令的
    ...
    def get_df(self, query_obj: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        ...
        print(timestamp_format,'打印的时间戳这个')
        # The datasource here can be different backend but the interface is common
        #数据源可能输不同的但是接口是通用的
        #datasource是外界传入到这个类的数据源的接口
        self.results = self.datasource.query(query_obj)#获取查询结果
        ...
        # parsing logic.
        #将我们从数据库收到的时间戳转换为pandas支持的时间格式。如果没有指定
        #python_date_format，则该模式将被视为默认的 ISO 日期格式如果日期格
        #式为 unix，解析将使用相应的解析逻辑。
        if not df.empty:
    ...
    def df_metrics_to_num(self, df):
        """当pandas.read_sql不能时吧刻度值转化成数字时，用这个函数将指标转换为数字"""
    ...
    def query_obj(self) -> Dict[str, Any]:
        ...
        #     pass
        # else:
        #     print("试试看能不能打印出数据源的类型")
        self.process_query_filters()#清洗表单数据
        #print('打印的from表单',form_data)
        ...
        if DTTM_ALIAS in groupby:#DTTM_ALIAS = "__timestamp"在utils中定义了
        ...
        #print("测试",timeseries_limit_metric)
        ...
        #额外内容用于查询数据源类型所特定的元素，例如仅适用于表的额外条款
        extras = {
    ...
    @property #@property 装饰器，可以直接通过方法名来访问方法，不需要在方法名后添加一对“（）”小括号。
    def cache_timeout(self):#cache数据库查询超时
    ...
    def get_json(self):#json.dumps()函数是将一个Python数据类型列表进行json格式的编码
    ...
    def cache_key(self, query_obj, **extra):
        ...
        我们删除硬值的日期时间界限，代之以使用提供的边界输入，这些输入可能是与时间相
        关的（如"5 天前"或"现在"）。
        ...
        "额外"参数当前被时间移位查询所使用，因为不同的时移只会在剥离的"from_dttm"和
        "to_dttm"值中有所不同。
        """
    ...
    def get_payload(self, query_obj=None):
        ...
        df = payload.get("df")
        if self.status != utils.QueryStatus.FAILED:# 调用get_data处理payload中查询到的df数据转化成chart需要的data
    ...
    def get_df_payload(self, query_obj=None, **kwargs):
        ...
        #print('打印的cache——key',cache_key)#看来返回的是如后所示的时间戳e7b05f310f577da81151965d348a611d
        #上面那个是处理extras额外查询中和时间相关的查询语句的，一般用不到
        logger.info("Cache key: {}".format(cache_key))
        is_loaded = False
        stacktrace = None
        df = None
        #print('query_obj打印的',query_obj)
        #下面那句是获取当前时间的意思
        cached_dttm = datetime.utcnow().isoformat().split(".")[0]
        #print('测试 ',cache)返回的是一个对象，写在utils/cache_manager中
        if cache_key and cache and not self.force:
            #print('进入了这个cache中')#测试证明要执性
            cache_value = cache.get(cache_key)
            #print('测试cache_value',cache_value)#测试证明是None
            if cache_value:
        ...
        if query_obj and not is_loaded:#上一步的if没执行就执行这个
            try:
                df = self.get_df(query_obj)#获取dataframe形式的查询结果
        ...
        #print('打印的额query',self.query)
        #print(df)
        if isinstance(df,dict):
            rowcount = max(len(df['nodes']),len(df['links']))#每次只能传回节点或者数据的一种，选一个大的有内容的即可
        else:
    ...
    def json_dumps(self, obj, sort_keys=False):#转化成json字符串数据
        return json.dumps(
    ...
    @property
    def json_data(self):#还是吧pythondata转换成json字符串
...
"""
#自己加的echart雷达图
class echartsRader(BaseViz):
    viz_type = 'echarts_radar' #对应前端的名字
    verbose_name = _('echarts_radar') #对应在网页上显示的chart名字
    is_timeseries = False #是否按时间排序

    def get_data(self,df):#获取query查询的数据并处理成填充图表js需要的数据字典返回
        data = self.handle_js_int_overflow(
            dict(
        ...
        columns = [{'name': columns_pre[i],'max':str(max_num)}for i in range(len(columns_pre))]    
        out_data = [
...
class NetGraphViz(NVD3Viz):
    ...
    def get_df(self,query_obj:Optional[Dict[str, Any]] = None) -> list:
        if not query_obj:
            query_obj = self.query_obj()#若输入参数丢失则重新获取query字典
        if not query_obj:
            return pd.DataFrame()
...
class LiuTuViz(NVD3Viz):
    ...
    def fun(self,str_num):
        if len(str_num.split('e'))>1 :#是科学计数法的处理，不是的直接float()
    ...
    def to_series(self, df, classed=""):
        ...
        size_data_num = []
        time = []
        for j in range(0,len(self.groupby)):
            for i in df[self.groupby[j]]:
                if self.groupby[j] == 'name':
                    name.append(i)
                elif self.groupby[j] == 'fillcolor':
                    fillcolor.append(i)
                elif self.groupby[j] == 'size_data':
                    size_data.append(i)
                elif self.groupby[j] == 'time':
                    time.append(i)
        for i in range(0,len(size_data)):
            size_data[i] = size_data[i].replace('[','')
            size_data[i] = size_data[i].replace(']','')
            size = size_data[i].split(",")  #拆分字符串成列表
            for j in range(0,len(size)):  #把列表里面的科学计数法字符串挨个转化成数字
                size[j] = self.fun(size[j])
            size_data_num.append(size) #存储的是每一时刻内的每个name的size切片
            size = []
        time_size = []
        b = []
        count = []
        size_data_sort = []
        for i in time:
            if i not in time_size:
                time_size.append(i)
                count.append(1)
            else:
                for n in range(0,len(time_size)):
                    if i == time_size[n]:
                        count[n] = count[n]+1
        #判断时序是否一致并正确
        try :
        ...
        except Exception as e:#这里可以改成让程序终止
            print('ERROR：', e)
        ...
        #初始化切片数组
        for i in b:
            size_data_sort.append([])

        print(count)
        for i in range(0,len(size_data_num)):
            for j in range(0,len(b)):
                if name[i] == b[j]:
                    size_data_sort[j].append(size_data_num[i][0])


        chart_predata = []
        for i in range(0,len(size_data_sort)):
            chart_predata.append({
            'id': 0,
            'name': None,
            'fillcolor': None,
            'size':None    
        })#layer字典的定义不能用先定义layer字典再append（layer）那样会迷之出错，全赋值成最后一次循环的，python循环第二个坑
            chart_predata[i]['id'] = i
            chart_predata[i]['name'] = name[i*len(time_size)]
            chart_predata[i]['fillcolor'] = fillcolor[i*len(time_size)]
            chart_predata[i]['size'] = size_data_sort[i]
...
class MixLineBarViz(NVD3Viz):
    ...
    def query_obj(self):
        ...
        intersection = [m for m in bar_metrics if m in line_metrics]#查询两种元素是否相同
        if intersection:
...
#最后是viz_types的定向引用把？，添加新的图表类一定要在这一段之前添加
viz_types = {
    o.viz_type: o
