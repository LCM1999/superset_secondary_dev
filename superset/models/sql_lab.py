...
class Query(Model, ExtraJSONMixin):
    ...
    database = relationship(#外联到Database这个orm对应的表，外键索引是id
        "Database",
        foreign_keys=[database_id],
        backref=backref("queries", cascade="all, delete-orphan"),#backref代表在Database类中动态创建属性指向queries表
    )#cascade决定了修改父表时，什么时候子表要进行级联操作。"all, delete-orphan"表示真正的all所有操作都要级联操作
