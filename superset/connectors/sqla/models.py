...
class SqlaTable(Model, BaseDatasource):
    ...
    def select_star(self) -> str:
        ...
        #调用的是链接的dbs对应的orm对象的select_star函数
    ...
    def get_query_cypher_str_extended(self,query_obj: Dict[str, Any]) -> QueryStringExtended:
        ...
        nodes_label_subject = query_obj['nodes_label_subject']
        nodes_label_comp = query_obj['nodes_label_comp']
        links_label_subject = query_obj['links_label_subject']
        links_label_comp = query_obj['links_label_comp']        
        cypher = ""
        match = "MATCH"
        the_return = "RETURN"
        interval = ' '
        ...
        if links_label_comp:
            label = "[r:`"+links_label_comp+"`]"
        cypher = match+interval+path+'='+from_node+label+to_node+interval+the_return+interval+path+',r'
        # if label_expected:
        #     if label_expected == 'nodes':
        #         cypher_obj = 'n'
        #         if label_comp:
        #             if label_subject == 'nodes_label':
        #                 label = "(n:`"+label_comp+"`)"
        #         else:
        #             label = "("+cypher_obj+")"
        #         cypher = match+interval+label+interval+the_return+interval+cypher_obj
        #     elif label_expected == 'links':
        #         cypher_obj = 'p'
        #         from_node = '()-'
        #         to_node = '->()'
        #         if label_comp:
        #             if label_subject == 'links_label':
        #                 label = "[r:`"+label_comp+"`]"
        #         else:
        #             label = ''
        #         cypher = match+interval+cypher_obj+'='+from_node+label+to_node+interval+the_return+interval+cypher_obj+',r'#返回一下relastionship
        # else:
        #     raise Exception(_("Empty query object(请选则要查询的对象)?"))
    ...
    def get_query_str(self, query_obj: Dict[str, Any]) -> str:
        if self.database.database_kind:
            query_str_ext = self.get_query_cypher_str_extended(query_obj)
        else:
            query_str_ext = self.get_query_str_extended(query_obj)
        all_queries = query_str_ext.prequeries + [query_str_ext.sql]#查询到的数据和查询语句
        return ";\n\n".join(all_queries) + ";"
    ...
    def query(self, query_obj: Dict[str, Any]) -> QueryResult:
        qry_start_dttm = datetime.now()#查询开始时间
        ...
        #构建查询语句并执行，返回的是一个带查询字段名字、预查询内容和查询语句的东西
        print('要判断的值',self.database.database_kind)
        if self.database.database_kind:#自己添加的处理不同数据库的东西
            print('判断成功了')
            query_str_ext = self.get_query_cypher_str_extended(query_obj)
            print('打印的构建查询语句',query_str_ext)
            ...
            try:
                print('进入到try中了')
                json_data = self.database.get_json(sql, self.schema, mutator)
                print("查询时间打印",datetime.now() - qry_start_dttm)
            except Exception as e:
                df = pd.DataFrame()
                status = utils.QueryStatus.FAILED
