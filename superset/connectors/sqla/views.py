...
class TableModelView(DatasourceModelView, DeleteMixin, YamlExportMixin):
    ...
    add_columns = ["database", "schema", "table_name"]#"query_language",#添加一个修改查询语句的东西表示可以修改
    edit_columns = [
        "table_name",
        "sql",
    ...
    def pre_add(self, table):
        with db.session.no_autoflush:#组织session池中存储的东西刷新到数据库
            table_query = db.session.query(models.SqlaTable).filter(
                models.SqlaTable.table_name == table.table_name,
                models.SqlaTable.schema == table.schema,
                models.SqlaTable.database_id == table.database.id,
            )
            if db.session.query(table_query.exists()).scalar():#scalar()返回查询结果第一列的数值
                raise Exception(get_datasource_exist_error_msg(table.full_name))
                #full_name返回的是database_name, schema, datasource_name
