...
class BaseTemplateProcessor:  # pylint: disable=too-few-public-methods
    """Base class for database-specific(特定于数据库的) jinja context
...
template_processors = {}
#print('打印的全局变量',globals())
keys = tuple(globals().keys())#拿到全局变量字典的键值
for k in keys:#获取engine对应的名字和class
    o = globals()[k]
...
def get_template_processor(database, table=None, query=None, **kwargs):
    ...
    #print('打印的全局变量',database.backend)#template_processors中只有三個class
    #print(template_processor)#backend如果不是key中有的就返回BaseTemplateProcessor类
    #图数据库后端因为url的原因应该是‘http’
