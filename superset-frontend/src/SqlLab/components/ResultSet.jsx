...
export default class ResultSet extends React.PureComponent {
  ...
  render() {
    ...
    if (query.state === 'stopped') {
      ...
      let data;
      //console.log(query.cached,'显示的数据')
      if (this.props.cache && query.cached) {
        data = this.state.data;
      } else if (results && results.data) {
        data = results.data;
      }
      //console.log(data,length,'数据长度')前端返回的图数据库的信息会导致data.length为0
      console.log(results)
      if (results.if_neo4j){
        return(
        <>
    ...
    if (query.cached || (query.state === 'success' && !query.results)) {//没有拿到数据
