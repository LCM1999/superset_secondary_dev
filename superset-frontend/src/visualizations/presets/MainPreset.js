...
import MixLineBarChartPlugin from '../MixLineBar/MixLineBarChartPlugin';//自己新加的插件引入，新加的插件路径不在UI里面,别忘了加“;”
import LiuTuChartPlugin from '../LiuTu/LiuTuChartPlugin';
import NetGraphChartPlugin from '../NetGraph/NetGraphChartPlugin';
  ...
  constructor() {
    super({
      ...
      plugins: [
        ...
        new LiuTuChartPlugin().configure({key:'liutu'}),
        new NetGraphChartPlugin().configure({key:'net_graph'})//关键字确定，用在src/explore/components/controls/VizTypeControl.jsx中告知后端服务器选择，也用在了组件选择当中
      ],
    });
  }
}
