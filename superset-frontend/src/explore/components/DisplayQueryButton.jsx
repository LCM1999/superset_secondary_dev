...
export class DisplayQueryButton extends React.PureComponent {
  ...
  beforeOpen(endpointType) {
    console.log('latestQueryFormData',this.props.latestQueryFormData)
  ...
  renderResultsModalBody() {
    if (this.state.isLoading) {
      ...
      console.log('调用了这个去显示结果')
      if (this.state.data.length === 0) {
        return 'No data';
      }
      if (this.props.latestQueryFormData['viz_type'] == 'net_graph'){
        return this.renderDataJson(this.state.data)
      }
      else{
