...
export default function QueryAndSaveBtns({
  ...
  let flashButtonStyle = 'default';
  if (chartIsStale ){
    flashButtonStyle = 'primary';
  }
  else if (flashQuery){
    flashButtonStyle = 'success'
  }
  else if (!flashQuery){
    flashButtonStyle = 'default'
  }
  ...
  const saveButtonDisabled = errorMessage ? true : loading; //？前面值为真给变量赋值冒号前面的，否则赋值后面的
  const qryOrStopButton = loading && !flashQuery ? (
  ...
  const flashButton = (
    ...
    className="flash"
    onClick={onFlash}
    bsStyle={flashButtonStyle}
    disable={!!errorMessage}
    >
    <i className="fa fa-bolt"/> Flash
    </Button>
  )
