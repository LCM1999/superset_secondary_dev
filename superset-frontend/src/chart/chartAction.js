...
export function runAnnotationQuery(
  ...
  return function(dispatch, getState) {
    ...
    const controller = new AbortController();//形成一个新的请求终止控制器
    ...
    return SupersetClient.get({//原先的一次请求的方法，现在计划加入ajax长请求
      url,
      signal,
      ...
      .catch(response =>
        getClientErrorObject(response).then(err => {
          if (err.statusText === 'timeout') {
            ...
            dispatch(annotationQuerySuccess(annotation, err, sliceKey));//执行写入查询结果
...
export const FLASH_QUERY = 'FLASH_QUERY';
export function flashQuery(value = true,key){
  return { type: FLASH_QUERY,value,key};
}
...
export const CHART_STATE_FLASH = 'CHART_STATE_FLASH';
export function chartStateFlash(key){
  return{type:CHART_STATE_FLASH,key};
}
...
export function exploreJSON(
  ...
  method,
  dashboardId,
) {//只有dispatch()才能导致stroe变化，进而导致界面重新渲染
  return dispatch => {
    ...
    //console.log('看看这一步返回了什么东西',{ url, payload })，返回的是页面的url请求和处理过的formData里面的大部分数据
...
export function ceshi(key){
  return {type:CESHI,key};//console.log('chart_dispatch_test')
}
...
export function postChartFormData(
   ...
   *
   */
  //console.log('一次dashboardId',dashboardId)
  return exploreJSON(formData, force, timeout, key, 'POST', dashboardId);
}

export const FLASH_POST_CHART_FORM_DATA = 'FLASH_POST_CHART_FORM_DATA'
export function flashpostChartFromData(
  fromData,
){
  return {type:CESHI}
}
